...
function install() {
  return gulp.series(
    ...
    () => {
      ...
      return gulp.src(demoPackageJsonPath).pipe(
        gulpInstall({
          production: true
        })
      )
...
class BuildTask {
  ...
  init() {
    ...
    gulp.task(`${id}-component-check`, async () => {
      ...
      // console.log('此时componentListMap') = this.componentListMap
    })

    // NOTE 2020年7月28日 11:06:35新增，添加wxs文件
    /**
     * 写 wxs 文件到目标目录  2020年7月28日 10:50:56 NOTE:
     */
    gulp.task(`${id}-component-wxs`, (done) => {
      // const jsonFileList = this.componentListMap.jsonFileList
      const wxsFileList = config.wxsEntry

      if (wxsFileList && wxsFileList.length) {
